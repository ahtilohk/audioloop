...
class AudioRepository(private val context: Context) {
    ...
    private fun getNotesDir(): File = File(filesDir, ".notes").apply { if (!exists()) mkdirs() }
    private fun getNoteFile(audioFile: File): File = File(getNotesDir(), "${audioFile.name}.note.txt")
    ...
    suspend fun getAllRecordingsSync(): List<RecordingItem> {
        return dao.getAllRecords().map { it.toRecordingItem() }
    }

    suspend fun getRecordingsByCategorySync(category: String): List<RecordingItem> {
        // This is a bit tricky because we don't have a suspend version in DAO yet for category
        // But we can just use the Flow's first value or add it to DAO.
        // Let's add it to DAO for efficiency.
        return dao.getRecordingsByCategorySync(category).map { it.toRecordingItem() }
    }

    suspend fun insertRecording(item: RecordingItem, category: String, isPublic: Boolean = false): AudioResult<Unit> {
        return try {
    ...
    suspend fun updateNote(filePath: String, note: String): AudioResult<Unit> {
        return try {
    ...
    suspend fun renameRecording(item: RecordingItem, newName: String): AudioResult<File> {
        return try {
            val oldFile = item.file
            val ext = oldFile.extension
            val newFile = File(oldFile.parent, if (ext.isNotEmpty()) "$newName.$ext" else newName)
            
            if (newFile.exists()) return AudioResult.Error("File already exists")
    ...
    suspend fun discoverRecordings(categories: List<String>): AudioResult<Unit> {
        return try {
    ...
    suspend fun deleteRecording(path: String): AudioResult<Unit> {
        return try {
            val file = File(path)
            ...
            if (file.exists() && !file.delete()) {
                return AudioResult.Error("Failed to delete file from disk")
            }
    ...
    suspend fun importFromPublicStorage(): AudioResult<Int> {
        if (android.os.Build.VERSION.SDK_INT < android.os.Build.VERSION_CODES.Q) return AudioResult.Success(0)
