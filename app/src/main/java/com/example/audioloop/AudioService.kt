...
class AudioService : Service() {
    ...
    private suspend fun startMicRecording(fileName: String, source: Int, usePublic: Boolean, category: String) {
        ...
        withContext(Dispatchers.Main) {
            startForegroundNotification(NOTIFICATION_ID_RECORDING, "Microphone recording...", false, true)
        }
    ...
    private suspend fun startInternalRecording(fileName: String, resultCode: Int, data: Intent) {
        ...
        withContext(Dispatchers.Main) {
            startForegroundNotification(NOTIFICATION_ID_RECORDING, "Stream recording active...", true, true)
        }
    ...
    fun playFile(item: RecordingItem, speed: Float, pitch: Float) {
        stopPlayback()
        isPlaying = true
        startForegroundNotification(NOTIFICATION_ID_PLAYBACK, "Playing: ${item.name}", false, false)
    ...
    private fun onPlaybackComplete() {
        ...
        sendBroadcast(Intent("com.example.audioloop.PLAYBACK_COMPLETE").setPackage(packageName))
    }
